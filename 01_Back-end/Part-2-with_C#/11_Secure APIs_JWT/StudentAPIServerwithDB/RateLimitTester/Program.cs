    ...
    static async Task Main()
    {
        ...
        for (int i = 1; i <= 10; i++)
        {
            try
            {
                ...
                content = new StringContent(json, Encoding.UTF8, "application/json");//why ? because HttpContent can only be sent once, so we need to recreate it for each attempt.
            }
            catch (Exception ex)
            {
